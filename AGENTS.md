...
5.  **FORCE UPDATE:** Use the `--force-update` flag to bypass the 24-hour update check limit during development or testing.
6.  **GIST SYNC:** The sync mechanism relies on `sync_config.json` (gitignored). 
    - NEVER hardcode the Personal Access Token (PAT) or Gist ID.
    - If modifying sync logic, ensure it remains asynchronous (threaded) to prevent UI blocking.
    - Use `INSERT OR IGNORE` in SQL for cloud-to-local merging to prevent GUID collisions.
